...
>**NOTE** that even though DP's space complexity is almost linear when working with VCS, in extreme circumstances (e.g., countless alignments with no room for delta compression [as in `ABCABC..` vs `CBACBA..`]), *Versus* could take up to 64 GB of virtual memory. Beyond that limit (as you can read in the *Error handling* section [below](#error-handling)), the LCS/SES is just an estimate.
